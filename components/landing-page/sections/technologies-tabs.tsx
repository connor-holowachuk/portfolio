...
const TechnologiesTabs = () => {
	...
	return (
		<div className="grid grid-cols-2 lg:grid-cols-1 gap-2 w-auto float-left py-8">
			{TechnologiesOptions.map((option, index) => (
				<div
					key={index}
					onClick={() => setSelectedTab(option.title)}
					className={cn(
						'flex flex-row gap-2 items-center float-left h-12  transition-all duration-200 ease-in-out cursor-pointer px-4 color-white/50 justify-center md:justify-start w-auto rounded-xs hover:bg-accent-foreground',
